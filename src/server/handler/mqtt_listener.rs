...
pub fn mqtt_register_listener(mqtt_client: &mut impl MqttClientRepository, cfg: MqttConfig) {
    ...
    mqtt_client
        .subscribe(
            "test/test_message",
            Arc::new(|client: &AsyncClient, msg: &Message, data: &RequestContext| {
    ...
    mqtt_client
        .subscribe(
            ...
            Arc::new(move |client: &AsyncClient, msg: &Message, data: &RequestContext| {
                ...
                if  !data.card_repository().find_by_card_number(&*card.id).unwrap() {
